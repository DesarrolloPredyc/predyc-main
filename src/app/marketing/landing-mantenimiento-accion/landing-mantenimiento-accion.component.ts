...
@Component({
  selector: 'app-mantenimiento-accion',
  standalone: true,
 ...
 providers:[IconService ],
  templateUrl: './landing-mantenimiento-accion.component.html',
  styleUrl: './landing-mantenimiento-accion.component.css'
})
export class LandingMantenimientoAccionComponent implements OnInit{
  ...
  constructor(@Inject(PLATFORM_ID) private platformId: Object, public icon: IconService, private route: ActivatedRoute, private location: Location, private userService: UserService)
  {
  ...
  getInstructorNames(): string {
      return this.slides[this.currentIndex].instructors.map(i => i.name).join(', ');
  }

  getInstructorRoles(): string {
      return this.slides[this.currentIndex].instructors.map(i => i.role).join(', ');
  }
