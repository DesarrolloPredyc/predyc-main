import { compareByString } from "../../shared/utils";
...
export class Question {
  ...
  points: number = 0;
  newTrueFalseFormat:boolean = false;
	id: string;
  classId:string;
  explanation:string
  ...
  getDisplayText(): string {
    ...
    for (const placeholder of placeholders) {
      ...
      const placeholderHtml = `<select>${optionsHtml}</select>`;
...
export class Activity {
  ...
  isTestProfile:  boolean = false;
  isTestDiplomado:  boolean = false;
